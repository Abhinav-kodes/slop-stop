...
- ## ⚠️ The Problem: "Slopsquatting" & Account Hijacking
  ...
  Attackers monitor AI outputs, see which fake package names models hallucinate, and register those names on npm or PyPI filled with malware. When an AI suggests the package and writes the import, your environment is compromised the moment the file is saved or executed—even before you run `npm install`. Furthermore, phished maintainer accounts publishing malicious updates to established packages (e.g. `axios`, `is`, `keyv`) pose an equal threat.

---

- ## 🛡️ The Solution: Deterministic Agent-Agnostic Defense
  Slop-Stop provides a multi-layer defense stack built around **deterministic filesystem and shell boundaries**. It works across **all** AI coding tools (Antigravity, opencode, Cursor, Windsurf, Claude Code):
...
4. **The Backstop (Shell Shims):** Optional `slop-stop shim-npm` & `shim-pip` wrapper functions to intercept `npm install` and `pip install` commands before network requests occur.
...
| Package Status      | What it means                                                                                              | Layer 1: MCP Server            | Layer 2: File Watcher            | Layer 3: Git Pre-Commit Hook                 |
| :------------------ | :--------------------------------------------------------------------------------------------------------- | :----------------------------- | :------------------------------- | :------------------------------------------- |
| **`PASS`**          | Legitimate, established package or allowlisted/private scope.                                               | AI proceeds.                   | Silent / Green tick.             | ✅ Commit succeeds (`exit 0`).                |
| **`SUSPICIOUS`**    | Package exists, but has red flags (created < 14 days ago, 1 version, low OpenSSF Scorecard, no Sigstore). | AI warned to find alternative. | Massive **Yellow ⚠️ WARNING**.   | ⚠️ Commit succeeds, but loud warning printed. |
| **`HALLUCINATION`** | Package does not exist on registry (404).                                                                  | AI told not to write import.   | Massive **Red 🚨 INTERCEPTED 🚨**. | ❌ **Hard Block**: Commit fails (`exit 1`).  |
...
└────────┬──────────────────────────────┬───────────────────────────┬─────┘
         │ 1. MCP Handshake (Advisory)  │ 2. Writes file to disk    │ 3. Git Commit
         ▼                              ▼                           ▼
┌─────────────────┐           ┌──────────────────┐         ┌────────────────┐
